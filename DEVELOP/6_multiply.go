...
func main() {
	...
	http.ListenAndServe(":8080", nil)   // запускаем сервер для приёма запросов на порту 8080. При этом программа перейдёт в режим ожидания, а нам надо будет любым способом, хоть с браузера, обратиться к порту 8080
